  ...
  module Dsc
    class PowerShellManager
      ...
      def initialize(cmd, debug)
        ...
        Puppet.debug "dsc: #{Time.now} #{cmd} is running as pid: #{@ps_process[:pid]}"
        ...
        Puppet.debug "dsc: #{Time.now} PowerShell initialization complete for pid: #{@ps_process[:pid]}"
      ...
      def exit
        ...
        Puppet.debug "dsc: PowerShellManager exiting..."
...
Invoke-PowerShellUserCode @params
      ...
      def read_from_pipe(pipe, timeout = 0.1, &block)
        if self.class.is_readable?(pipe, timeout)
          ...
          Puppet.debug "dsc: #{Time.now} PIPE> #{l}"
      ...
      def read_streams
        ...
        Puppet.debug "dsc: Waited #{Time.now - start_time} total seconds."
